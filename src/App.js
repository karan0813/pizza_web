...
function App() {
  ...
  return (
    <>
      <Navbar />
      <div className=" grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1  m-5 bg-white rounded-sm shadow-2xl w-fit mx-auto ">
        {pizaData.map(({url,caption,id,price  }, index) => (
          <div key={id} className="m-10 shadow-xl  ">
            <div className=" rounded-t-lg ">
             ...
             <div className=" p-2 space-y-2   w-full  bg-[#EE8F03] ">
              <h1 className="text-xl font-semibold">{caption}</h1>
                 <p className="bg-blue-300 flex items-center px-3 rounded-xl  w-fit">5 <span><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                 <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg></span>
                  
                </p>
                <h1 className="font-semibold">₹ {price[0].small}/-</h1>
                <button className="px-5 py-1 bg-[#36A136] hover:border-red-600 border border-red-400  text-black font-semibold rounded-md cursor-pointer"  onClick={() => (
                  setclick(true),
                  setList(true),
                  setpricesmall(price[0].small),
                  setpricemedum(price[0].meduim),
                  setpricelarg(price[0].large),
                  setfulldata({url,caption,id,price })
      ...
      {List && <ListSize fulldata={fulldata} setList={setList} pricesmall={pricesmall} pricemedum={pricemedum} pricelarg={pricelarg}/>}
    </>
  );
}
